...
-- 2select hire_date ,first_name, last_name from employees;

-- 3select concat('first_name','last_name',',  ',job_id) as 'employee and title' from employees;

-- 4select hire_date ,first_name, last_name from employees
-- where job_id = 'pu_clerk';

-- 5select concat(employee_id,',',first_name,',',last_name,',',email,',',phone_number,',',hire_date,',',job_id,',',salary) as output from employees;

-- 6select first_name, last_name,salary from employees
-- where salary>2000

-- 7select concat(first_name, last_name) as name ,hire_date as start_date from employees;

-- 8select hire_date ,first_name, last_name from employees
-- order by hire_date;

-- 9select first_name, last_name,salary from employees
-- order by salary desc;

-- 10select first_name, last_name ,department_id ,salary from employees
...
-- 12select last_name,job_id,salary from employees 
-- where job_id in ('sa_rep' , 'st_clerk' )
-- and salary not in('2500','3500','5000');

-- 13select min(salary),max(salary),avg(salary),sum(commission_pct) from employees

-- 14select department_id,sum(salary),sum(commission_pct) from employees
-- group by department_id

-- 15 select department_id, count(employee_id) from employees
-- group by department_id 

-- 16select department_id,sum(salary) from employees
 -- group by department_id

-- 17 select first_name,last_name ,commission_pct from employees
-- where commission_pct is null

-- 20select manager_id ,sum(salary) from employees
-- group by manager_id
...
-- employee which has max salary
select first_name, last_name,salary from employees
order by salary desc
limit 1;

-- details of joining date of employees in company
select concat(first_name, last_name) as name ,hire_date as start_date from employees;

-- information of employees who do not  take commission from company
 select first_name, last_name ,department_id ,salary,commission_pct from employees
 where commission_pct is  null
 order by salary ;
 
 -- employee who has maximum commission 
  select first_name, last_name ,department_id ,salary,commission_pct from employees
 where commission_pct is not null
 order by commission_pct desc
 limit 1;

 -- numbers of employees work in specific department
select department_id, count(employee_id) from employees
group by department_id ;

-- distribution of salary done by specific manager to their employees who work under him
select manager_id ,sum(salary) from employees
group by manager_id;

-- avg salary of employees department wise
select department_id , avg(salary) from employees
group by department_id; 

-- information about the employees who work in which department
 select first_name,last_name ,department_name from employees
 join  departments
 on employees.department_id=departments.department_id;
 
 
 -- address info of employeess
  select employees.first_name ,employees.last_name ,departments.department_name, locations.location_id,locations.city,locations.country_id from employees
join departments
on employees.department_id=departments.department_id 
join locations
on departments.location_id=locations.location_id;

select sum(e.salary), c.country_name
from employees e
